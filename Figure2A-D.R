...
#799/1192 - 16S
idx <- df$primers %in% c("799F/1192R")
df2 <- df[idx, ]

#Plot nuage de points
a <-  ggplot(df2, aes(x=concentration, y=value, color=compartment)) + 
  geom_point() + 
  geom_smooth(method=lm, formula = y~x)+
  scale_color_brewer(palette="Dark2") + 
  stat_regline_equation(size= labelsize,
          aes(label =  paste(..eq.label.., ..rr.label.., sep = "~~~~"))) +
  labs(title = "16S - 799/1192",y=yaxis, x=xaxis)+
  scale_x_continuous(trans='log10') +
  scale_y_continuous(trans='log10') +
  theme_RTN2 
    
ggsave(paste(figures.dir, "16S.pdf", sep=""), a, width=10, height=10)
...
#Plot nuage de points
c <-  ggplot(df2, aes(x=concentration, y=value, color=compartment)) + 
  geom_point() + 
  geom_smooth(method=lm, formula = y~x)+
  scale_color_brewer(palette="Dark2") + 
  stat_regline_equation(size= labelsize,
          aes(label =  paste(..eq.label.., ..rr.label.., sep = "~~~~"))) +
  labs(title = "Plant ITS", y=yaxis, x=xaxis)+
  scale_x_continuous(trans='log10') +
  scale_y_continuous(trans='log10') +
  theme_RTN2 
    
ggsave(paste(figures.dir, "pITS.pdf", sep=""), c, width=10, height=10)
...
p12 <- plot_grid(a,b,c,d, labels = c('A','B','C','D'),label_size = 30, ncol=2, nrow = 2)

ggsave(paste(figures.dir, run, "Complete.pdf", sep=""),width=20, height=20, p12)